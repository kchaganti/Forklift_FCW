...
option.ttc                  = 3;                % In seconds
option.camerafps            = 4.64;             % In Hz
option.lookaheadframes      = ceil(option.ttc*option.camerafps); % We predict object motion and camer motion for these many frames ahead of time

% Read a video file and play it. 
videoFReader = VideoReader('C:\Users\kalyani.chaganti\Downloads\color.avi');
%%%%videoFReader_fisheye1 = VideoReader('C:\Users\kalyani.chaganti\Downloads\fisheye1.avi');
videoFReader_depth = VideoReader('C:\Users\kalyani.chaganti\Downloads\depth.avi');
dObject = load('C:\Users\kalyani.chaganti\Forklift_FCW\more_accurate_depth_and_odom.mat');
inObject = load('C:\Users\kalyani.chaganti\Forklift_FCW\imu_and_indices.mat');
odObject = load('C:\Users\kalyani.chaganti\Forklift_FCW\odom.mat');
framecount =1;

videoPlayer = vision.VideoPlayer;
while hasFrame(videoFReader)
      I = read(videoFReader,framecount);
      I = imresize(I,[480,640]);
      I_depth = read(videoFReader_depth,dObject.depth_frame_index2(framecount)+1)
 %     I_fisheye1 = read(videoFReader_fisheye1,inObject.fisheye1_index(framecount)+1);
      ...
      % Assign depths to the strongest bounding boxes.
 %%     [depths] = assignDepthstoBoundingBoxes(bboxes, I_depth);
      ...
      % Optional object bounding box display
%         if (~isempty(bboxes))
...
%         end
      ...
      [assignments, unassignedTracks, unassignedDetections] = ...
       ...
       deleteLostTracks();    
       createNewTracks();
       % Send track info to depth Kalman filter
    
       displayTrackingResults();
       framecount = framecount+1;
       %I = imresize(I,[480,640]);
       %I_rescale_fisheye = imresize(I_fisheye1,[480,640]);
       %size(I)
       %size(I_depth)
       step(videoPlayer, imfuse(I,I_depth,'blend'));
       step(videoPlayer, I);
end
release(videoPlayer);
release(videoFReader);
%%%%%release(videoFReader_fisheye1);
release(videoFReader_depth);
%% Assign depths to bounding boxes
% We assign depths to bounding boxes

    function [depths] = assignDepthstoBoundingBoxes(bboxes, I_depth)
        depths = [];

        for i = 1:size(bboxes,1)
            %depth = min value of image in bounding box
            depth = min(I_depth(bboxes(i,2):(bboxes(i,2) + bboxes(i,4)-1),bboxes(i,1): (bboxes(i,1) + bboxes(i,3)-1)));
            depths = [depths;depth]; %#ok<AGROW>
        end
    end
...
%                           height].
% * |depths| :              An N-by-1 vector to record the depth
%                           by matching to the depth image
...
% based on the previous detection scores. 
    ...
    function updateAssignedTracks()
        ...
        for i = 1:numAssignedTracks
            ...
            %Populate track's depth
            tracks(trackIdx).depths = [tracks(trackIdx).depths; depths(detectionIdx)];
...
% was not assigned to a track.
    ...
    function updateUnassignedTracks()
        for i = 1:length(unassignedTracks)
            ...
            tracks(idx).depths = [tracks(idx).depths; 0];
